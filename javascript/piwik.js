  ...
  window.Matomo = window.Piwik = (function() {
    ...
    function U(ct, cn) {
      ...
      function db() {
        ...
        var dL = new Date(), dJ = Math.round(dL.getTime() / 1000), dK = a2("id"), dO = aL(dK), dN, dM;
        if (dO) {
          dN = dO.split(".");
          dN.unshift("0");
          if (b0.length) {
      ...
      function dz() {
        var dT, dM = new Date(), dN = Math.round(dM.getTime() / 1000), dY, dL, dO = 1024, dV, dP, dK = a2("ses"), dS = a2("ref"), dR = aL(dK), dJ = bX(), dX = bf || bZ, dU, dQ, dW = {};
        dU = dJ[0];
        dQ = dJ[1];
        dY = dJ[2];
        dL = dJ[3];
      ...
      function cL(dK, dW, dX) {
        var dV, dJ = new Date(), dU = aZ, dQ = a2("cvar"), dZ = bf || bZ, dL = cr(dZ);
        if (bx) {
          aN()
        }
        if (dd) {
      ...
      function bD(dM, dL, dQ, dN, dJ, dT) {
        var dP = "idgoal=0", dK = new Date(), dR = [], dS, dO = String(dM).length;
        if (dO) {
          dP += "&ec_id=" + u(dM)
        }
        dP += "&revenue=" + dL;
